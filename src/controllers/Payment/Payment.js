...
async function notifyPaymentUsers({ userId, title, body, data, notifyAdmins = false }) {
...
export const adminUpdatePayment = async (req, res) => {
  try {
    ...
    if (uploaded?.url) {
      ...
      saved = await Payment.findByIdAndUpdate(id, update, { new: true }).lean();
    ...
    // 🔔 NOTIFY PAYMENT OWNER
    await notifyPaymentUsers({
      userId: current.userId,
      title: "✏️ Payment Updated",
      body: `Your payment of ₹${saved.amount} was updated`,
      data: {
