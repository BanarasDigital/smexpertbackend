...
export const sendGroupMessage = async (req, res) => {
  try {
    ...
    // Populate sender & group (for notification text)
    ...
    for (const t of tokens) {
      await sendToToken(t.token, {
        notification: {
          title: group.name,
          body: `${sender.name}: ${message.content || "📎 Attachment"}`,
        },
        data: {
          type: "group_chat",
...
export const sendGroupMessageController = async (req, res) => {
  try {
    ...
    // (Optional) Verify sender is a member of the group
    const group = await GroupConversation.findById(groupId).select("name members admin");
    if (!group) {
      return res.status(404).json({ success: false, message: "Group not found" });
    }
    const isMember = String(group.admin) === String(senderId) 
                  || (group.members || []).some(m => String(m) === String(senderId));
    if (!isMember) {
      return res.status(403).json({ success: false, message: "Sender is not a member of this group" });
    }

    // Prepare any file attachments (already uploaded to S3 in middleware)
    let attachments = [];
    if (Array.isArray(req.s3Uploads) && req.s3Uploads.length > 0) {
      attachments = req.s3Uploads.map(f => ({
        url: f.url,
        name: f.originalName || f.name,
        type: f.type,
        size: f.size
    ...
    const newMessage = await GroupMessage.create({
      ...
      isReadBy: [senderId]  // mark as read by sender
    });
    // Update group conversation's last-updated time (for ordering chats lists)
    await GroupConversation.updateOne(
      { _id: groupId },
      { $set: { updatedAt: new Date() } }
    );
    const populatedMessage = await newMessage.populate("senderId", "name email profileImage");
    const senderName = populatedMessage.senderId.name;
    const io = getIo();
    io.to(`group:${groupId}`).emit("receive_group_message", populatedMessage);
    const title = group.name; 
    const bodyText = content || (attachments.length ? "📎 Attachment" : ""); 
    const notificationBody = `${senderName}: ${bodyText}`; 
    const allMemberIds = [...group.members.map(m => m.toString()), group.admin.toString()];
    const recipientIds = allMemberIds.filter(uid => uid !== senderId.toString());
    const tokens = await DeviceToken.find({ "meta.userId": { $in: recipientIds } });
    const payload = {
      data: {
        title: title,
    ...
    for (const device of tokens) {
      await sendToToken(device.token, payload);
    }

    return res.status(201).json({ success: true, message: populatedMessage });
  } catch (error) {
    console.error("sendGroupMessageController error:", error);
